...
def find_player_position(player,category):
    ...
    player_matches=Matches.objects.filter(category=category).filter(query).order_by('-match_index')
...
def get_player_ranking(player,category,ranking_list):
   ...
   player_ranking=RankedPlayers.objects.filter(list=ranking_list).filter(category__name=category.category).filter(player=player)
...
def update_ranking_of_registered_players(tournament_category,ranking_list,grade):
    ...
    for player in player_list:
        ...
        if ranking==None:
            rank_category=RankingListCategories.objects.get(name=tournament_category.category)
            ranking=RankedPlayers(list=ranking_list,category=rank_category,player=player)
            ranking.save()
...
def update_ranking_according_to_tournament_records(tournament):
    ...
    tournament_categories=TournamentCategories.objects.filter(tournamet=tournament)
...
def update_players_ranking(ranking_list, category):
        ...
        players_list = RankedPlayers.objects.filter(list=ranking_list).filter(category=category).order_by('-points')
        rank = 1
